...
public class Sudoku
{
	...
	{
		this.sudoku = java.util.Arrays.stream(sudoku.sudoku).map(el -> el.clone()).toArray($ -> sudoku.sudoku.clone());
		emptyCells = sudoku.getEmptyCells();
		solutionSpace = sudoku.getSolutionSpace();
		solutionCounter = sudoku.getSolutionCounter();
	}
